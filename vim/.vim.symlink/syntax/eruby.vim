...
syn cluster erubyRegions contains=erubyOneLiner,erubyBlock,erubyExpression,erubyComment,ppErubyOneLiner,ppErubyBlock,ppErubyExpression,ppErubyComment
...
exe 'syn region  ppErubyOneLiner   matchgroup=ppErubyDelimiter start="^%\{1,'.b:eruby_nest_level.'\}%\@!"    end="$"     contains=@rubyTop	     containedin=ALLBUT,@erubyRegions keepend oneline'
exe 'syn region  ppErubyBlock      matchgroup=ppErubyDelimiter start="\[%\{1,'.b:eruby_nest_level.'\}%\@!-\=" end="[=-]\=%\@<!%\{1,'.b:eruby_nest_level.'\}\]" contains=@rubyTop  containedin=ALLBUT,@erubyRegions keepend'
exe 'syn region  ppErubyExpression matchgroup=ppErubyDelimiter start="\[%\{1,'.b:eruby_nest_level.'\}=\{1,4}" end="[=-]\=%\@<!%\{1,'.b:eruby_nest_level.'\}\]" contains=@rubyTop  containedin=ALLBUT,@erubyRegions keepend'
exe 'syn region  ppErubyComment    matchgroup=ppErubyDelimiter start="\[%\{1,'.b:eruby_nest_level.'\}#"       end="%\@<!%\{1,'.b:eruby_nest_level.'\}\]" contains=rubyTodo,@Spell containedin=ALLBUT,@erubyRegions keepend'

" Define the default highlighting.

hi def link erubyDelimiter		PreProc
hi def link ppErubyDelimiter		Type
